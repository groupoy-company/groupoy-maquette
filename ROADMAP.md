...
## FAZ B-bis — Simplification & élégance (en cours)
> Décidé le 4 août après analyse : 50 écrans pour **212 entrées de menu**, dont 31 % occupées par 4 écrans génériques répétés. L'application est rangée comme l'organigramme, pas comme le travail. Aucune fonctionnalité n'est supprimée — on enlève des portes, pas des pièces.

1. [x] **Cohérence des thèmes** (fait 04/08) — **530 couleurs claires écrites en dur** remplacées par les variables du thème ou des teintes translucides (fonds, bordures, teintes d'alerte, dégradés), sur 49 écrans + App.jsx. Avant : en thème « Obsidienne & Or », des écrans entiers restaient blancs (Facturation externe, Budget, Tableau de bord…). Après : **0 fond clair écrit en dur** dans tout le projet ; mesuré à l'écran, il reste 4 éléments clairs sur 567 (0,7 %, internes aux graphiques). Les couleurs de sens (rouge/orange/vert) sont devenues translucides : elles gardent leur signification sur fond clair **comme** sur fond sombre. Le texte blanc sur bouton coloré n'a pas été touché. Outil réutilisable : `tools/theme_sweep.py`. Un vrai bug a été attrapé au passage (8 variables de thème utilisées hors du composant → plantage) et corrigé.
2. [x] **Écran « Aujourd'hui »** (fait 04/08) — l'écran d'arrivée après connexion. Répond à « qu'attend-on de moi maintenant ? ». Ordre choisi selon le **coût de l'inaction** : d'abord les retards et échéances proches (coût réel et croissant), puis ce qui attend une validation (on bloque quelqu'un), puis les chiffres (repliés), puis les dernières activités. Un **bandeau fin** en haut donne le pouls (CA, chantiers, AO, effectif) sans prendre la place de l'action. **Adapté au rôle** : direction = vue groupe ; finance = assurances/contrats/factures ; RH = conformité ; exploitation = véhicules/conformité/AO.
3. [x] **Fusion des écrans en double** (fait 04/08) — vérification faite avant d'agir : les deux écrans ne se recouvraient qu'à **26 %** (« Statistiques » contient 3 analyses propres à un service : études de prix, études, affaires). **Rien n'a donc été supprimé** : les deux contenus sont réunis dans un seul écran **Indicateurs** à deux onglets — « Vue d'ensemble » et « Détail du service ». Résultat : une entrée de menu au lieu de deux (15 entrées retirées, 212 → 203), zéro perte. Les 4 écrans génériques (Indicateurs, Planning, Calendrier, Processus) sont désormais regroupés **en fin de menu** sous l'intertitre « Outils du service », pour que les écrans métier ressortent en haut.
4. [x] **Renommages** (fait 04/08) : Présentation → *Modèle Ruches* · Présentation Group → *Vitrine Groupe* · Suivi → *Performance Essaim* · Suivi des Dossiers AO → *Dossiers AO* · Suivi Prestataires → *Missions Prestataires*.
5. [x] **Recherche des enregistrements** (fait 04/08) — ⌘K trouve maintenant les **fiches** autant que les écrans : chantiers, appels d'offres, collaborateurs, véhicules (marque, modèle, **immatriculation**), contrats, assurances, sociétés. Résultats groupés « Fiches » / « Écrans ». À l'ouverture d'une fiche, l'application **se place toute seule dans la bonne entité et le bon service** puis sélectionne l'enregistrement. Vérifié : « colmar » → contrat Mairie de Colmar → *Ezel › Juridique › Contrats* ; « GJ-110 » → BMW Série 1 → *La Roulotte › Parc & Matériel › Parc Automobile*.
6. [ ] **GED par dossier** : brancher le panneau Documents sur chaque fiche (une affaire, une étude, un chantier, un salarié) pour ne plus jamais ouvrir Drive. **Commencer par les études** (structure standard : RC, CCTP, CCAP, DPGF, plans, MT, DC1/DC2), puis les affaires.
7. [ ] **Notifications** : cloche in-app (immédiat) → résumé e-mail hebdomadaire (avec Resend) → notification téléphone (avec la PWA).
8. [ ] **Journal des modifications** : qui a changé quoi, quand (indispensable à plusieurs utilisateurs).
9. [ ] **Touches de plaisir** : accueil enrichi selon la semaine, courte animation à la victoire d'un AO, transitions douces entre écrans, abeille de chargement.
