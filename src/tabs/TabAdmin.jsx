  ...
  return (
          <>
            <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
              <h2 style={{fontSize:'1.3rem', fontWeight:700, color:$text, display:'flex', alignItems:'center', gap:8, color:"#8B6F47"}}>⚙️ Administration</h2>
              <div style={{display:'flex', gap:8}}>
                ...
                <button onClick={() => setAdminTab('structure')} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${adminTab==='structure' ? 'bg-amber-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>🏛️ Structure</button>
                        ...
                        return (
                          <tr key={u.id} className={idx%2===0?'':'bg-gray-50'}>
                            ...
                            {ordreOnglets.map(oid => {
                              ...
                              const permLabel = perm === 'write' ? '✏️' : perm === 'read' ? '👁️' : '🚫';
                              return (
                                <td key={oid} className="p-1 border text-center">
                                  {canEditThisUser ? (<select className={`text-xs rounded px-1 py-0.5 font-bold border-0 cursor-pointer ${permColor}`} value={perm} onChange={e => { const newPerm = e.target.value; const updatedUser = {...u, permissions: {...u.permissions, [oid]: newPerm}}; setUsers(prev => prev.map(x => x.id === u.id ? updatedUser : x)); if (currentUser.id === u.id) setCurrentUser(updatedUser); }}><option value="write">✏️ Écriture</option><option value="read">👁️ Lecture</option><option value="hidden">🚫 Caché</option></select>) : <span className={`text-xs rounded px-2 py-0.5 font-bold ${permColor}`}>{permLabel}</span>}
                                </td>
                              );
                            })}
                          </tr>
            ...
            {adminTab === 'structure' && isSuperAdmin() && (
              <div style={{display:'flex', flexDirection:'column', gap:24}}>
                <h3 style={{fontSize:'1rem', fontWeight:700, color:$text}}>🏛️ Rattachement des filiales aux holdings</h3>
