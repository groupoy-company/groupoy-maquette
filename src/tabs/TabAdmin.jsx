                        ...
                        return (
                          <tr key={u.id} className={idx%2===0?'':'bg-gray-50'}>
                            ...
                            {ordreOnglets.map(oid => {
                              ...
                              const permLabel = perm === 'write' ? '✎' : perm === 'read' ? '◎' : '∅';
                              return (
                                <td key={oid} className="p-1 border text-center">
                                  {canEditThisUser ? (<select className={`text-xs rounded px-1 py-0.5 font-bold border-0 cursor-pointer ${permColor}`} value={perm} onChange={e => { const newPerm = e.target.value; const updatedUser = {...u, permissions: {...u.permissions, [oid]: newPerm}}; setUsers(prev => prev.map(x => x.id === u.id ? updatedUser : x)); if (currentUser.id === u.id) setCurrentUser(updatedUser); }}><option value="write">✎ Écriture</option><option value="read">◎ Lecture</option><option value="hidden">∅ Caché</option></select>) : <span className={`text-xs rounded px-2 py-0.5 font-bold ${permColor}`}>{permLabel}</span>}
                                </td>
                              );
                            })}
                          </tr>
