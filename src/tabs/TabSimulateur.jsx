...
export default function TabSimulateur(__props) {
  const { $accent, $bgCard, $bgCardHover, $bgSub, $border, $borderLight, $danger, $info, $selBg, $selText, $success, $text, $textMut, $textSec, $warn, achatsST, amortissements, baremeRelatif, baseVariable, ca, coeffResult, coefficient, collaborateurs, crmRd, ebePercent, ebeValeur, ecartEBE, filNom, filialeData, filialeSelectionnee, filiales, formatEuro, formatPercent, fraisGroupOY, fraisHolding, fraisInternes, fraisInternesPercent, fraisYilmaz, getCoefficient, grille, impots, margeBrute, margeBrutePercent, niveau, niveauSelectionne, remunerationAvantPlafond, remunerationTotale, resultatExploitation, resultatNet, resultatNetPercent, setCa, setCollabSelectionne, setFilialeSelectionnee, setFraisInternes, setNiveauSelectionne, setOngletActif, setSimCompare, setSimPrintSel, setSimScenarioNom, setSimScenarios, setSimTab, setSousTraitance, setTauxAmortissements, setTauxFraisGroupOY, setTauxFraisHolding, setTauxFraisYilmaz, simCompare, simPrintSel, simScenarioNom, simScenarios, simTab, sousTraitance, tauxAmortissements, tauxFraisGroupOY, tauxFraisHolding, tauxFraisYilmaz, tauxImpots, totalFraisStructure, variableAjustee, variableModulee } = __props;
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:24, marginBottom:28}}>
              <div style={{background:$bgCard, borderRadius:crmRd, padding:28, boxShadow:'0 2px 16px rgba(0,0,0,0.03)', border:`1px solid ${$border}`}}>
                ...
                <div style={{display:'flex', flexDirection:'column', gap:16}}>
                  ...
                  <div style={{background:$accent+"06",borderRadius:crmRd,padding:16,border:`1px solid ${$accent}30`}}><label style={{display:'block', fontSize:'0.92rem', fontWeight:600, color:$textSec, marginBottom:6}}>◆ Frais GROUP OY</label><div style={{display:'flex', gap:8}}><input type="number" value={Math.round(tauxFraisGroupOY * 100 * 10) / 10} onChange={(e) => setTauxFraisGroupOY(Number(e.target.value) / 100)} style={{flex:1, padding:'12px 16px', border:'1.5px solid #e5e0d8', borderRadius:crmRd, outline:'none', fontSize:'0.98rem'}} step="0.5" min="0" max="3" /><div style={{display:'flex', flexDirection:'column', gap:0}}><button onClick={() => setTauxFraisGroupOY(Math.min(0.03, tauxFraisGroupOY + 0.005))} style={{padding:'4px 12px', background:$accent, color:'white', borderRadius:'8px 8px 0 0', fontWeight:700, fontSize:'1rem', lineHeight:1, border:'none', cursor:'pointer'}}>▲</button><button onClick={() => setTauxFraisGroupOY(Math.max(0, tauxFraisGroupOY - 0.005))} style={{padding:'4px 12px', background:$accent, color:'white', borderRadius:'0 0 8px 8px', fontWeight:700, fontSize:'1rem', lineHeight:1, border:'none', cursor:'pointer'}}>▼</button></div></div><p style={{fontSize:'0.92rem', color:$textSec, marginTop:4}}>{formatPercent(tauxFraisGroupOY)} = {formatEuro(fraisGroupOY)}</p><p style={{fontSize:'0.82rem', color:$textMut}}>Consolidation, supervision</p></div>
                </div>
              </div>
              <div style={{display:'flex',flexDirection:'column',gap:16}}>
              <div style={{background:$bgCard, borderRadius:crmRd, padding:28, boxShadow:'0 2px 16px rgba(0,0,0,0.03)', border:`1px solid ${$border}`}}>
          ...
          {simTab==='graphiques'&&(<div style={{display:'flex',flexDirection:'column',gap:20}}>
            ...
            <div style={{background:$bgCard,borderRadius:crmRd,padding:24,border:`1px solid ${$border}`}}>
              <div style={{fontWeight:700,fontSize:'1rem',color:$text,marginBottom:16,display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:'1.2rem'}}>§</span> Barème coefficient — EBE réalisé vs cible</div>
          ...
          {simTab==='classement'&&(<div style={{display:'flex',flexDirection:'column',gap:16}}>
            ...
            {simAlertes.length>0&&(
                <div style={{padding:'12px 16px',borderRadius:crmRd,background:$warn+'10',border:`1px solid ${$warn}30`}}>
                  <div style={{fontWeight:700,fontSize:'0.82rem',color:$warn,marginBottom:8}}>▲ {simAlertes.length} alerte{simAlertes.length>1?'s':''}</div>
