...
export default function TabPlanningGantt(__props) {
  const { $bg, $bgCard, $bgSub, $border, $text, $textMut, $textSec, PLAN_PHASES, PLAN_STATUTS, crmRd, navEntreprise, navService, planBarColor, planColWidths, planDetailId, planFiltrePhase, planFiltrePrio, planFiltreResp, planFiltreStatut, planGanttScale, planGanttZoom, planGroupBy, planGroupesFermes, planKanbanDrag, planOffset, planProjets, planSearch, planVue, planZoom, setPlanBarColor, setPlanColWidths, setPlanDetailId, setPlanFiltrePrio, setPlanFiltreResp, setPlanGroupBy, setPlanGroupesFermes, setPlanKanbanDrag, setPlanOffset, setPlanProjets, setPlanSearch, setPlanVue, setPlanZoom } = __props;
          ...
          if (navEntreprise === 'ezel' && navService === 'etudes_prix') {
            ...
            const AO_PHASES_PL = [
              {id:'preparation',label:'PRÉPARATION',color:ACC,bg:'rgba(240,236,253,0.14)',icon:'🔧'},
              {id:'soumission',label:'SOUMISSION',color:'#ea580c',bg:'rgba(255,243,238,0.14)',icon:'📤'},
              {id:'resultat',label:'RÉSULTAT',color:'#059669',bg:'rgba(232,245,238,0.14)',icon:'🏆'},
            ];
            const getPhaseAO = (s) => {
            ...
            const TM_PL = {'Marché public':{c:'#0055cc',bg:'rgba(224,240,255,0.14)',e:'🏛️'},'Marché privé':{c:'#166534',bg:'rgba(34,197,94,0.14)',e:'🏢'},'Marché particulier':{c:'#7c3aed',bg:'rgba(139,92,246,0.14)',e:'🏠'}};
            ...
            return (<div style={{display:'flex',flexDirection:'column',gap:16}}>
              ...
              {aoUrgentsPL.length > 0 && (
                <div style={{background:'rgba(239,68,68,0.10)',border:'1px solid #fecaca',borderRadius:crmRd,padding:'12px 16px'}}>
                  ...
                  <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
                    {aoUrgentsPL.map(a => { const dl=dlLeft(a.d); return (
                      <div key={a.id} style={{padding:'5px 10px',borderRadius:crmRd,background:$bgCard,border:'1px solid #fecaca',display:'flex',alignItems:'center',gap:6}}>
              ...
              {planVue === 'tableau' && (
                <div style={{background:$bgCard,border:`1px solid ${$border}`,borderRadius:crmRd,overflow:'auto'}}>
                  <table style={{width:'100%',borderCollapse:'collapse',fontSize:'0.79rem',minWidth:900}}>
                    ...
                    <tbody>
                      {filteredPL.map((ao,i) => { const dl=dlLeft(ao.d); const sc=SC_PL[ao.s]||'#9ca3af'; const phId=getPhaseAO(ao.s); const phCfg=AO_PHASES_PL.find(p=>p.id===phId)||{color:$textSec,bg:$bgSub,icon:'🗄️',label:'ARCHIVÉ'}; const tmCfg=TM_PL[ao.tm]||{c:'#6b7280',bg:$bgSub,e:'📋'}; const aoBrief=ao.n.replace(/^\d{4}\.\d{2}\.\d{2} - /,'').slice(0,52);
                        return (
                    ...
                    return (
                      <div key={phase.id} style={{background:$bgCard,border:`1px solid ${$border}`,borderRadius:crmRd,overflow:'hidden'}}>
                        ...
                        <div style={{padding:'8px',display:'flex',flexDirection:'column',gap:5,maxHeight:460,overflowY:'auto'}}>
                          ...
                          {pfiltAOs.map(ao => { const dl=dlLeft(ao.d); const sc=SC_PL[ao.s]||'#9ca3af'; const tmCfg=TM_PL[ao.tm]||{c:'#6b7280',bg:$bgSub,e:'📋'}; const aoBrief=ao.n.replace(/^\d{4}\.\d{2}\.\d{2} - /,'').slice(0,42);
                            return (
          ...
          return (<div style={{fontFamily:'-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'}} onClick={() => setPlanDetailId(null)}>
            ...
            <div style={{display:'grid', gridTemplateColumns:'repeat(5, 1fr)', gap:12, marginBottom:20}}>
              {[
                ...
                <div key={i} style={{background:$bgCard, borderRadius:crmRd, padding:'14px 16px', border:`1px solid ${$border}`, borderTop:`3px solid ${k.color}`}}>
              ...
              {planVue === 'gantt' && <>
                <div style={{display:'flex', gap:2, background:$bgSub, borderRadius:crmRd, padding:2}}>
                  {['jour','semaine','mois','trimestre'].map(z => (
                    <button key={z} onClick={() => setPlanZoom(z)} style={{padding:'4px 8px', borderRadius:crmRd, fontSize:'0.75rem', fontWeight:planZoom===z?600:400, background:planZoom===z?$bgCard:'transparent', color:planZoom===z?'#ea580c':'#b0a08a', border:planZoom===z?`1px solid ${$border}`:'1px solid transparent', cursor:'pointer', textTransform:'capitalize'}}>{z}</button>
                  ))}
                </div>
                <button onClick={() => setPlanOffset(o=>o-1)} style={{padding:'4px 8px', borderRadius:crmRd, fontSize:'0.8rem', border:`1px solid ${$border}`, background:$bgCard, cursor:'pointer'}}>◀</button>
                <button onClick={() => setPlanOffset(0)} style={{padding:'4px 10px', borderRadius:crmRd, fontSize:'0.75rem', fontWeight:600, border:'1px solid #fed7aa', background:'rgba(249,115,22,0.10)', color:'#ea580c', cursor:'pointer'}}>Aujourd'hui</button>
                <button onClick={() => setPlanOffset(o=>o+1)} style={{padding:'4px 8px', borderRadius:crmRd, fontSize:'0.8rem', border:`1px solid ${$border}`, background:$bgCard, cursor:'pointer'}}>▶</button>
              </>}
            </div>
            {/* Filter bar */}
            <div style={{display:'flex', alignItems:'center', gap:8, marginBottom:16, flexWrap:'wrap'}}>
              <input value={planSearch} onChange={e => setPlanSearch(e.target.value)} placeholder="🔍 Rechercher..." style={{padding:'5px 10px', borderRadius:crmRd, fontSize:'0.82rem', border:`1px solid ${$border}`, background:$bgCard, width:160, outline:'none'}} />
              <select value={planFiltrePrio} onChange={e => setPlanFiltrePrio(e.target.value)} style={{padding:'5px 10px', borderRadius:crmRd, fontSize:'0.82rem', border:`1px solid ${$border}`, background:$bgCard, cursor:'pointer', outline:'none'}}>
              ...
              <select value={planFiltreResp} onChange={e => setPlanFiltreResp(e.target.value)} style={{padding:'5px 10px', borderRadius:crmRd, fontSize:'0.82rem', border:`1px solid ${$border}`, background:$bgCard, cursor:'pointer', outline:'none'}}>
              ...
              {planVue !== 'kanban' && <select value={planGroupBy} onChange={e => { setPlanGroupBy(e.target.value); setPlanGroupesFermes([]); }} style={{padding:'5px 10px', borderRadius:crmRd, fontSize:'0.82rem', border:`1px solid ${$border}`, background:$bgCard, cursor:'pointer', outline:'none'}}>
              ...
              {planVue === 'gantt' && <select value={planBarColor} onChange={e => setPlanBarColor(e.target.value)} style={{padding:'5px 10px', borderRadius:crmRd, fontSize:'0.82rem', border:`1px solid ${$border}`, background:$bgCard, cursor:'pointer', outline:'none'}}>
            ...
            {planVue === 'gantt' && (
              <div style={{background:$bgCard, borderRadius:crmRd, border:`1px solid ${$border}`, overflow:'hidden'}}>
                <div style={{display:'flex', overflow:'hidden'}}>
                  {/* Left panel */}
                  <div style={{width:260, flexShrink:0, borderRight:'2px solid #f0ebe3', background:$bgCard}}>
                    ...
                    {getGrouped().map(grp => {
                      ...
                      return (<div key={grp.key}>
                        ...
                        {!isFerme && grp.items.map(p => (
                          <div key={p.id} onClick={e => { e.stopPropagation(); setPlanDetailId(p.id); }} style={{padding:'6px 12px', borderBottom:`1px solid ${$border}`, cursor:'pointer', height:40, display:'flex', flexDirection:'column', justifyContent:'center', background: planDetailId===p.id?'rgba(249,115,22,0.10)':'transparent'}} className="hover:bg-gray-50">
                      ...
                      {(() => {
                        ...
                        return (<div style={{borderBottom:`2px solid ${$border}`, background:$bgSub}}>
                          ...
                          <div style={{display:'flex', height:24, borderBottom:`1px solid ${$border}`}}>
                            {topGroups.map((g,gi) => (
                              <div key={gi} style={{width:g.span * zc.cellW, minWidth:g.span * zc.cellW, textAlign:'center', fontSize:'0.72rem', fontWeight:800, color:$textSec, lineHeight:'24px', borderRight:'1px solid #e2e8f0', background: gi%2===0?$border:$bgSub, letterSpacing:'0.03em'}}>{g.label}</div>
                            ))}
                          </div>
                          ...
                          <div style={{display:'flex', height:28}}>
                            {Array.from({length:zc.cols}).map((_,i) => {
                              ...
                              return (<div key={i} style={{width:zc.cellW, minWidth:zc.cellW, textAlign:'center', borderRight:`1px solid ${$border}`, background: isToday?'rgba(249,115,22,0.10)':isWe?$bgCard:'transparent', display:'flex', flexDirection:'column', justifyContent:'center', position:'relative'}}>
                      ...
                      {getGrouped().map(grp => {
                        ...
                        return (<div key={grp.key}>
                          ...
                          {!isFerme && grp.items.map(p => {
                            ...
                            return (<div key={p.id} style={{position:'relative', height:40, borderBottom:`1px solid ${$border}`, background: planDetailId===p.id?'rgba(212,160,48,0.12)':'transparent'}}>
                              {/* Grid columns */}
                              {Array.from({length:zc.cols}).map((_,i) => (<div key={i} style={{position:'absolute', left:i*zc.cellW, top:0, bottom:0, width:zc.cellW, borderRight:'1px solid #faf8f5', background: zc.isWeekend(i)?$bgCard:'transparent'}} />))}
                              {/* Bar */}
              ...
              <div style={{background:$bgCard, borderRadius:crmRd, border:`1px solid ${$border}`, overflow:'hidden'}}>
                {getGrouped().map(grp => {
                  ...
                  return (<div key={grp.key} style={{borderBottom:`1px solid ${$border}`}}>
                    {planGroupBy !== 'none' && <div onClick={() => setPlanGroupesFermes(prev => prev.includes(grp.key) ? prev.filter(k=>k!==grp.key) : [...prev, grp.key])} style={{display:'flex', alignItems:'center', gap:8, padding:'8px 16px', background:$bgCard, cursor:'pointer', userSelect:'none', borderBottom: isFerme?'none':`1px solid ${$border}`}}>
                      <svg width="8" height="8" viewBox="0 0 8 8" style={{transform:isFerme?'rotate(-90deg)':'rotate(0deg)', transition:'0.15s'}}><path d="M1.5 2l2.5 3 2.5-3" fill={grp.color}/></svg>
                    ...
                    {!isFerme && (<table style={{width:'100%', borderCollapse:'collapse'}}>
                      ...
                      <tbody>{grp.items.map(p => {
                        ...
                        return (<tr key={p.id} onClick={e => { e.stopPropagation(); setPlanDetailId(p.id); }} style={{cursor:'pointer', borderBottom:`1px solid ${$border}`, background: planDetailId===p.id?'rgba(212,160,48,0.12)':'transparent'}} className="hover:bg-gray-50">
                    ...
                    <div style={{flex:1, overflowY:'auto', padding:8, display:'flex', flexDirection:'column', gap:8}} className="hide-scrollbar">
                      {items.map(p => {
                        ...
                        return (<div key={p.id} draggable onDragStart={() => setPlanKanbanDrag(p.id)} onDragEnd={() => setPlanKanbanDrag(null)}
                          ...
                          style={{background:$bgCard, borderRadius:crmRd, padding:'12px 14px', border:`1px solid ${$border}`, cursor:'grab', boxShadow:'0 1px 3px rgba(0,0,0,0.04)', transition:'box-shadow 0.15s', borderLeft:`3px solid ${st.color}`}}>
                          <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:6}}>
                            ...
                            <span style={{fontSize:'0.68rem', padding:'1px 5px', borderRadius:3, background: p.type==='Public'?'rgba(59,130,246,0.14)':'#fce7f3', color: p.type==='Public'?'#2563eb':'#db2777', fontWeight:600}}>{p.type}</span>
                          </div>
            ...
            {/* ════ PANNEAU DÉTAIL ════ */}
            {detailProjet && (<div onClick={e => e.stopPropagation()} style={{position:'fixed', top:0, right:0, bottom:0, width:380, background:$bgCard, boxShadow:'-4px 0 20px rgba(0,0,0,0.08)', zIndex:100, overflowY:'auto', borderLeft:`1px solid ${$border}`, padding:'20px 24px'}}>
              <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16}}>
                <span style={{fontSize:'0.75rem', fontWeight:600, color:$textMut}}>{detailProjet.aoRef || detailProjet.id}</span>
                <button onClick={() => setPlanDetailId(null)} style={{width:28, height:28, borderRadius:crmRd, border:`1px solid ${$border}`, background:$bgCard, cursor:'pointer', fontSize:'0.9rem', display:'flex', alignItems:'center', justifyContent:'center'}}>✕</button>
              </div>
              ...
              <div style={{display:'flex', gap:6, flexWrap:'wrap', marginBottom:16}}>
                ...
                <span style={{fontSize:'0.75rem', padding:'3px 10px', borderRadius:crmRd, background: detailProjet.priorite==='Haute'?'rgba(239,68,68,0.14)':detailProjet.priorite==='Moyenne'?'rgba(212,160,48,0.18)':'rgba(59,130,246,0.14)', color: detailProjet.priorite==='Haute'?'#dc2626':detailProjet.priorite==='Moyenne'?'#d97706':'#2563eb', fontWeight:700}}>{detailProjet.priorite}</span>
              </div>
