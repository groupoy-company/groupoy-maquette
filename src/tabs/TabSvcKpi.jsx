...
export default function TabSvcKpi(__props) {
  const { $accent, $accentSub, $bgCard, $bgCardHover, $bgSub, $border, $borderLight, $shadow, $shadowLg, $text, $textMut, $textSec, AO_PHASES, AO_STATUTS, SERVICES_CONFIG, SourceLogo, aoColDragOverIdx, aoColOrder, aoPrioFiltre, aoStatutOrdre, aoSvcColWidths, aoTypeFiltre, appelsOffres, crmRd, getAoGrouped, getAoStats, handleAoColDrop, navEntreprise, navService, setAoColDragIdx, setAoColDragOverIdx, setAoPrioFiltre, setAoSvcColWidths, setAoTypeFiltre, setOngletActif, startColResize } = __props;
          ...
          if (navEntreprise === 'ezel' && navService === 'etudes_prix') {
            ...
            const byType = [
              { label:'Marché public', key:'pub', count: raw.filter(a=>a.tm==='Marché public').length, gagnes: gagnes.filter(a=>a.tm==='Marché public').length, color:'#0055cc', bg:'#e0f0ff', e:'◆' },
            ...
            const kpis = [
              ...
              { label:'Visites à planifier', val: raw.filter(a=>['Visite rdv à prendre','Visite demandée'].includes(a.s)).length, sub:`terrain + obligatoires`, icon:'◆', color:'#0891b2', bg:'#0891b212' }
            ];
          ...
          const SVC_KPI_LABEL = { etudes_prix: '◺ Suivi Études', preparation: '☰ Suivi Préparation', execution: '◆ Suivi Financier', cloture: '✓ Suivi Réception', comptabilite: '€ Suivi Compta', administratif: '▤ Suivi Admin' };
          const dynamicSvcKpiLabel = SVC_KPI_LABEL[navService] || 'Réponses AO';

          const SERVICE_KPI = {
            'ezel_etudes_prix': { title: "Bureau d'Études & Prix — Ezel Bâtiment", kpis: [{ label: 'Dossiers en cours', value: '14', icon: '◺', color: '#3498db' },{ label: 'Devis déposés (mois)', value: '7', icon: '✓', color: '#2ecc71' },{ label: 'Délai moyen étude', value: '12j', icon: '◷', color: '#e67e22' },{ label: 'Taux succès AO', value: '42%', icon: '▦', color: '#9b59b6' }] },
            'ezel_preparation': { title: 'Préparation Chantiers — Ezel Bâtiment', kpis: [{ label: 'Chantiers en préparation', value: '5', icon: '✱', color: '#e67e22' },{ label: 'Plans validés (mois)', value: '8', icon: '✓', color: '#2ecc71' },{ label: 'Commandes fournisseurs', value: '22', icon: '▣', color: '#3498db' },{ label: 'Délai moyen prépa', value: '8j', icon: '◷', color: '#9b59b6' }] },
            'ezel_execution': { title: 'Suivi Financier Chantiers — Ezel Bâtiment', kpis: [{ label: 'Chantiers actifs', value: '6', icon: '◆', color: '#e67e22' },{ label: 'Avancement moyen', value: '62%', icon: '↗', color: '#2ecc71' },{ label: 'Retards signalés', value: '2', icon: '▲', color: '#e74c3c' },{ label: 'Heures travaillées', value: '3 240h', icon: '◷', color: '#3498db' }] },
            'ezel_cloture': { title: 'Clôture Affaires — Ezel Bâtiment', kpis: [{ label: 'Affaires à clôturer', value: '4', icon: '🏁', color: '#9b59b6' },{ label: 'DGD signés (mois)', value: '3', icon: '✓', color: '#2ecc71' },{ label: 'Réserves levées', value: '87%', icon: '▦', color: '#3498db' },{ label: 'Délai paiement solde', value: '38j', icon: '◷', color: '#e67e22' }] },
            'ezel_comptabilite': { title: 'Comptabilité — Ezel Bâtiment', kpis: [{ label: 'Factures à émettre', value: '18', icon: '↥', color: '#3498db' },{ label: 'Factures à payer', value: '24', icon: '↧', color: '#e74c3c' },{ label: 'Encours client', value: '3.2M€', icon: '▬', color: '#e67e22' },{ label: 'DSO moyen', value: '45j', icon: '◷', color: '#9b59b6' }] },
            'ezel_administratif': { title: 'Administratif — Ezel Bâtiment', kpis: [{ label: 'Documents en attente', value: '7', icon: '▫', color: '#e67e22' },{ label: 'Contrats actifs', value: '23', icon: '✎', color: '#3498db' },{ label: 'Assurances à renouveler', value: '2', icon: '⬟', color: '#e74c3c' },{ label: 'Conformité URSSAF', value: '100%', icon: '✓', color: '#2ecc71' }] },
            ...
            'roulotte_planification': { title: 'Planification — La Roulotte', kpis: [{ label: 'Interventions planifiées', value: '28', icon: '◫', color: '#3498db' },{ label: 'Disponibilité flotte', value: '25 unités', icon: '▸', color: '#2ecc71' },{ label: 'Conflits planning', value: '3', icon: '▲', color: '#e74c3c' },{ label: 'Taux ponctualité', value: '92%', icon: '◷', color: '#9b59b6' }] },
            'roulotte_comptabilite': { title: 'Comptabilité — La Roulotte', kpis: [{ label: 'Factures clients à émettre', value: '11', icon: '↥', color: '#3498db' },{ label: 'Factures fournisseurs à payer', value: '8', icon: '↧', color: '#e74c3c' },{ label: 'Encours client', value: '680k€', icon: '▬', color: '#e67e22' },{ label: 'Délai paiement moyen', value: '38j', icon: '◷', color: '#9b59b6' }] },
            ...
            'echafaudage_exploitation': { title: 'Exploitation & Montage — L\'Échafaudage', kpis: [{ label: 'Chantiers montage actifs', value: '8', icon: '✱', color: '#6C3483' },{ label: 'Tonnes en location', value: '320t/450t', icon: '▣', color: '#3498db' },{ label: 'Montages cette semaine', value: '5', icon: '◆', color: '#e67e22' },{ label: 'Démontages planifiés', value: '3', icon: '↻', color: '#2ecc71' }] },
            ...
            'echafaudage_comptabilite': { title: 'Comptabilité — L\'Échafaudage', kpis: [{ label: 'Factures clients à émettre', value: '16', icon: '↥', color: '#3498db' },{ label: 'Factures fournisseurs à payer', value: '19', icon: '↧', color: '#e74c3c' },{ label: 'Encours client', value: '1.8M€', icon: '▬', color: '#e67e22' },{ label: 'Délai paiement moyen', value: '52j', icon: '◷', color: '#9b59b6' }] },
            'echafaudage_administratif': { title: 'Administratif — L\'Échafaudage', kpis: [{ label: 'Documents en attente', value: '5', icon: '▫', color: '#e67e22' },{ label: 'Contrats actifs', value: '34', icon: '✎', color: '#3498db' },{ label: 'Habilitations à renouveler', value: '4', icon: '⬟', color: '#e74c3c' },{ label: 'Conformité sécurité', value: '95%', icon: '✓', color: '#2ecc71' }] },
            'etancheite_execution': { title: "Suivi Financier Chantiers — L'Étanchéité", kpis: [{ label: 'Chantiers actifs', value: '4', icon: '🔨', color: '#0e6655' },{ label: 'Avancement moyen', value: '55%', icon: '↗', color: '#2ecc71' },{ label: 'Retards signalés', value: '1', icon: '▲', color: '#e74c3c' },{ label: 'M² traités (mois)', value: '2 800m²', icon: '◺', color: '#3498db' }] },
            ...
            'etancheite_preparation': { title: "Préparation Chantiers — L'Étanchéité", kpis: [{ label: 'Chantiers en prépa', value: '3', icon: '✱', color: '#e67e22' },{ label: 'Plans validés (mois)', value: '4', icon: '✓', color: '#2ecc71' },{ label: 'Commandes matériaux', value: '11', icon: '▣', color: '#3498db' },{ label: 'Délai moyen prépa', value: '6j', icon: '◷', color: '#9b59b6' }] },
            'etancheite_cloture': { title: "Clôture Affaires — L'Étanchéité", kpis: [{ label: 'Affaires à clôturer', value: '2', icon: '🏁', color: '#9b59b6' },{ label: 'DGD signés (mois)', value: '2', icon: '✓', color: '#2ecc71' },{ label: 'Réserves levées', value: '91%', icon: '▦', color: '#3498db' },{ label: 'Délai paiement solde', value: '42j', icon: '◷', color: '#e67e22' }] },
            'etancheite_technique': { title: "Bureau Technique — L'Étanchéité", kpis: [{ label: 'Études techniques en cours', value: '6', icon: '🛠️', color: '#3498db' },{ label: 'Rapports validés (mois)', value: '8', icon: '✓', color: '#2ecc71' },{ label: 'Délai moyen étude', value: '12j', icon: '◷', color: '#e67e22' },{ label: 'Diagnostics planifiés', value: '3', icon: '⌕', color: '#9b59b6' }] },
            'etancheite_comptabilite': { title: "Comptabilité — L'Étanchéité", kpis: [{ label: 'Factures à émettre', value: '7', icon: '↥', color: '#3498db' },{ label: 'Factures à payer', value: '11', icon: '↧', color: '#e74c3c' },{ label: 'Encours client', value: '420k€', icon: '▬', color: '#e67e22' },{ label: 'DSO moyen', value: '40j', icon: '◷', color: '#9b59b6' }] },
            'etancheite_administratif': { title: "Administratif — L'Étanchéité", kpis: [{ label: 'Documents en attente', value: '3', icon: '▫', color: '#e67e22' },{ label: 'Contrats actifs', value: '12', icon: '✎', color: '#3498db' },{ label: 'Certifications à renouveler', value: '1', icon: '⬟', color: '#e74c3c' },{ label: 'Conformité Qualibat', value: '100%', icon: '✓', color: '#2ecc71' }] },
            'roulotte_planification': { title: 'Planification — La Roulotte', kpis: [{ label: 'Interventions planifiées', value: '28', icon: '◫', color: '#3498db' },{ label: 'Disponibilité flotte', value: '25 unités', icon: '▸', color: '#2ecc71' },{ label: 'Conflits planning', value: '3', icon: '▲', color: '#e74c3c' },{ label: 'Taux ponctualité', value: '92%', icon: '◷', color: '#9b59b6' }] },
            'echafaudage_planification': { title: "Planification — L'Échafaudage", kpis: [{ label: 'Montages planifiés', value: '14', icon: '◫', color: '#6C3483' },{ label: 'Équipes disponibles', value: '4', icon: '◉', color: '#2ecc71' },{ label: 'Conflits planning', value: '1', icon: '▲', color: '#e74c3c' },{ label: 'Taux ponctualité', value: '89%', icon: '◷', color: '#3498db' }] }
          };
