  ...
  const { $accent, $bg, $bgCard, $bgCardHover, $bgSub, $border, $borderLight, $danger, $info, $selBg, $selText, $shadow, $success, $text, $textMut, $textSec, $warn, SERVICES_CONFIG, WidgetErrorBoundary, ajouterFiliale, amortissements, appelsOffres, ca, calculsFiliales, chantiers, collaborateurs, crmRd, dashGroupeVue, dashSettingsOpen, dashWidgetOrder, dashWidgetSizes, dashWidgets, dashboardChantierId, dashboardCollabId, dashboardFiliale, dashboardVue, dataEvolutionCA, defaultAnnees, defaultWidgetOrder, donneesAnneeActive, donneesAnneesSupp, donneesDragOverIdx, donneesFilialeOrder, donneesFinancieres, dragOverWidget, dragWidget, employes, emptyChantier, emptyEmploye, filNom, filiales, filialesDynamiques, filialesEnrichies, fraisInternes, getAlerts, getChantiersCollab, getDonnee, getEmployesFiliale, getKpiFiliale, handleDonneesDrop, handleSettingsDrop, handleWidgetDrop, hiddenServicesYilmaz, impots, isDragging, margeBrute, modalFilialeOuvert, navEntreprise, nbFiliales, niveau, nouvelleFiliale, pennylaneApiKey, pennylaneError, pennylaneStatus, resultatExploitation, resultatNet, setChantierForm, setCollabDetailTab, setCollabFiltreFiliale, setCollabOngletId, setConfirmDelete, setDashGroupeVue, setDashSettingsOpen, setDashWidgetOrder, setDashWidgets, setDashboardChantierId, setDashboardCollabId, setDashboardFiliale, setDashboardVue, setDonnee, setDonneesAnneeActive, setDonneesAnneesSupp, setDonneesDragIdx, setDonneesDragOverIdx, setDragOverWidget, setDragWidget, setEmployeForm, setModalChantier, setModalEmploye, setModalFilialeOuvert, setNavService, setNouvelleFiliale, setOngletActif, setPennylaneApiKey, setPennylaneError, setPennylaneStatus, setSettingsDragIdx, setSettingsDragOverIdx, setYilmazVue, settingsDragIdx, settingsDragOverIdx, showBorderAccent, sousTraitance, toggleWidget, toggleWidgetSize, totalEffectif, widgetDescriptions, widgetLabels, yilmazVue } = __props;
          ...
          <div style={{maxWidth:1200, margin:'0 auto'}}>
            ...
            {isHolding && (() => {
              ...
              return (<>
                ...
                <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:24, marginBottom:28}}>
                  ...
                  <div style={{background:$bgCard, borderRadius:crmRd, padding:28, border:`1px solid ${$border}`, boxShadow:'0 2px 16px rgba(0,0,0,0.03)'}}>
                    <h3 style={{fontSize:'1.1rem', fontWeight:700, color:$text, marginBottom:20, display:'flex', alignItems:'center', gap:8}}>🏛️ Structure {activeFiliale.nom}</h3>
                    <div style={{display:'flex', flexDirection:'column', gap:12}}>
                ...
                <div style={{background:$bgCard, borderRadius:crmRd, border:`1px solid ${$border}`, boxShadow:'0 2px 16px rgba(0,0,0,0.03)', overflow:'hidden'}}>
                  <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 24px', background:$bgCard}}><h3 style={{fontSize:'1rem', fontWeight:700, color:$text}}>🏢 Filiales de {activeFiliale.nom} ({children.length})</h3></div>
            ...
            {dashboardFiliale === 'yilmaz' && (() => {
              ...
              <div style={{display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:16, marginBottom:28}}>
                {[
                  ...
                  {icon:'🏗️', label:'FILIALES SERVIES', value: filialesDynamiques.filter(f => f.holding !== 'GROUP OY').length, sub:'opérationnelles', color:'#059669'},
              ...
              <div style={{background:$bgCard, borderRadius:crmRd, border:`1px solid ${$border}`, boxShadow:'0 2px 16px rgba(0,0,0,0.03)', marginBottom:24, overflow:'hidden'}}>
                <div style={{padding:'20px 24px', borderBottom:`1px solid ${$border}`, background:$bgCard, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
              ...
              <div style={{background:$bgCard, borderRadius:crmRd, border:`1px solid ${$border}`, boxShadow:'0 2px 16px rgba(0,0,0,0.03)', marginBottom:24, overflow:'hidden'}}>
                <div style={{padding:'20px 24px', borderBottom:`1px solid ${$border}`, background:$bgCard}}>
                  <div style={{display:'flex', alignItems:'center', gap:8}}>
                  ...
                  <table style={{width:'100%', borderCollapse:'collapse', fontSize:'0.92rem'}}>
                    ...
                    <tbody>
                      {[
                        ...
                        <tr key={bi} style={{borderBottom:`1px solid ${$border}`, background:bi%2===0?$bgCard:$bgCard}}>
            ...
            {dashboardFiliale === null && (() => {
              ...
              chantiers.forEach(ch => { if (ch.depense > ch.budgetHT * 0.9 && ch.avancement < 80) alertes.push({type:'danger', msg:`🏗️ ${ch.nom} — Budget dépassé à ${Math.round(ch.depense/ch.budgetHT*100)}% pour ${ch.avancement}% d'avancement`, fId: ch.filialeId}); if (ch.statut === 'En cours' && ch.avancement < 30) alertes.push({type:'warning', msg:`🏗️ ${ch.nom} — Avancement faible (${ch.avancement}%)`, fId: ch.filialeId}); });
              ...
              const renderWidget = (key) => {
                ...
                switch(key) {
                  ...
                  case 'alertes': return (<div {...dragProps(key)} style={dragWrapStyle(key)}><div style={dragHandleStyle} className="drag-grip">⠿</div><div style={cardStyle}><div style={headerStyle}><div style={{display:'flex',alignItems:'center',gap:8}}><div style={titleStyle}>🚨 Alertes & Points d'attention</div><span style={{fontSize:'0.8rem',fontWeight:700,background:alertes.some(a=>a.type==='danger')?'rgba(239,68,68,0.10)':'rgba(212,160,48,0.12)',color:alertes.some(a=>a.type==='danger')?'#dc2626':'#d97706',padding:'2px 8px',borderRadius:crmRd}}>{alertes.length}</span></div></div><div style={{padding:20,display:'flex',flexDirection:'column',gap:8}}>{alertes.map((a,i)=>{const colors=a.type==='danger'?{bg:'rgba(239,68,68,0.10)',border:'#dc2626',text:'#991b1b',icon:'▲'}:a.type==='warning'?{bg:'rgba(212,160,48,0.12)',border:'#f59e0b',text:'#92400e',icon:'●'}:{bg:'rgba(34,197,94,0.10)',border:'#059669',text:'#166534',icon:'✓'};return(<div key={i} onClick={()=>{if(a.fId)setDashboardFiliale(a.fId);}} style={{padding:'14px 18px',borderRadius:crmRd,background:colors.bg,borderLeft:`4px solid ${colors.border}`,cursor:a.fId?'pointer':'default'}}><span style={{fontSize:'0.84rem',color:colors.text}}>{colors.icon} {a.msg}</span></div>);})}</div></div></div>);
                    ...
                    <div style={cardStyle}>
                      ...
                      <div style={{padding:20}}>
                        <div style={{display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:12, marginBottom:16}}>
                          ...
                          <div style={{background:'rgba(212,160,48,0.12)', borderRadius:crmRd, padding:'14px 12px', textAlign:'center'}}>
                      ...
                      <div style={{overflowX:'auto'}}>
                        <table style={{width:'100%', borderCollapse:'collapse', fontSize:'0.92rem'}}>
                          ...
                          <tbody>
                            {benchmarks.map((b, bi) => (
                              <tr key={bi} style={{borderBottom:`1px solid ${$border}`, background: bi % 2 === 0 ? $bgCard : $bgCard}}>
                                <td style={{padding:'8px 14px', fontWeight:600, color:$textSec, position:'sticky', left:0, background: bi % 2 === 0 ? $bgCard : $bgCard, zIndex:1}}>{b.label}</td>
                                {allKpis.filter(f => f.kpi.ca > 0).map(f => {
                                  const val = b.getValue(f);
              ...
              <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20}}>
                <div style={{display:'flex', gap:8}}>
                  {[{key:'kpi', label:'📊 Tableau de Bord'}, {key:'donnees', label:'📋 Centre de Données'}].map(tab => (
                    <button key={tab.key} onClick={() => setDashGroupeVue(tab.key)}
                      style={{padding:'10px 24px', borderRadius:crmRd, fontSize:'0.98rem', fontWeight:700, border: dashGroupeVue === tab.key ? 'none' : `1px solid ${$border}`, background: dashGroupeVue === tab.key ? '#8B6F47' : $bgCard, color: dashGroupeVue === tab.key ? 'white' : '#6b5d4d', cursor:'pointer', transition:'all 0.3s', boxShadow: dashGroupeVue === tab.key ? '0 4px 16px rgba(139,111,71,0.2)' : '0 2px 8px rgba(0,0,0,0.02)'}}
                    >{tab.label}</button>
                  ))}
                </div>
                {dashGroupeVue === 'kpi' && (
                ...
                <button onClick={() => setDashSettingsOpen(!dashSettingsOpen)}
                  style={{display:'flex', alignItems:'center', gap:6, padding:'8px 16px', borderRadius:crmRd, border:`1px solid ${$border}`, background: dashSettingsOpen ? $bgSub : $bgCard, color:$textSec, fontSize:'0.92rem', fontWeight:600, cursor:'pointer', transition:'all 0.2s'}}
                    ...
                    {[...dashWidgetOrder, ...defaultWidgetOrder.filter(k => !dashWidgetOrder.includes(k))].map((key, idx) => {
                      ...
                      >
                        ...
                        <button onClick={(e) => { e.stopPropagation(); toggleWidgetSize(key); }}
                          ...
                          style={{border:`1px solid ${$border}`, background: dashWidgetSizes[key] === 'half' ? $bgSub : $bgCard, borderRadius:crmRd, padding:'2px 6px', cursor:'pointer', fontSize:'0.75rem', color:$accent, fontWeight:700, flexShrink:0}}
              ...
              {dashGroupeVue === 'donnees' && (() => {
                ...
                const inputStyle = {width:'100%', padding:'7px 10px', border:`1px solid ${$border}`, borderRadius:crmRd, fontSize:'0.92rem', textAlign:'right', background:$bgCard, outline:'none', color:$text, fontWeight:600, fontVariantNumeric:'tabular-nums', transition:'all 0.2s'};
                ...
                <div>
                    ...
                    <div style={{padding:'16px 20px', background: pennylaneStatus === 'connected' ? 'rgba(34,197,94,0.10)' : $bgSub, borderRadius:crmRd, marginBottom:16, border: pennylaneStatus === 'connected' ? '1px solid #bbf7d0' : '1px solid transparent', transition:'all 0.3s'}}>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              setPennylaneStatus('error');
                              setPennylaneError('⚠️ Connexion bloquée par le navigateur (CORS). Voir note ci-dessous.');
                            }
                          }}
                    ...
                    <div style={{display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:12}}>
                      {[
                        ...
                        <div key={i} style={{padding:'14px 16px', borderRadius:crmRd, border:`1px solid ${$border}`, background: api.status === 'active' ? 'rgba(34,197,94,0.10)' : api.status === 'configured' ? '#eef2ff' : $bgSub, transition:'all 0.2s'}}
            ...
            {dashboardFiliale !== null && dashboardFiliale !== 'yilmaz' && !isHolding && (() => {
              ...
              const fHS = {padding:'20px 24px', borderBottom:`1px solid ${$border}`, background:$bgCard};
              ...
              const statutBadge = (st) => { const m = {
                'En cours': {bg:'rgba(212,160,48,0.18)', color:'#92400e', border:'#fbbf24'},
                'Planifié': {bg:'rgba(59,130,246,0.14)', color:'#1e40af', border:'#60a5fa'},
                'Terminé': {bg:'rgba(34,197,94,0.14)', color:'#166534', border:'#4ade80'}
              ...
              // Active tab style
              const tabS = (active) => ({padding:'10px 24px', borderRadius:crmRd, fontSize:'0.98rem', fontWeight:700, border: active ? 'none' : `1px solid ${$border}`, background: active ? `linear-gradient(135deg, ${fG[0]}, ${fG[1]})` : $bgCard, color: active ? '#fff' : '#6b5d4d', cursor:'pointer', transition:'all 0.3s', boxShadow: active ? `0 4px 16px ${fG[0]}44` : '0 2px 8px rgba(0,0,0,0.02)', textShadow: active ? '0 1px 2px rgba(0,0,0,0.2)' : 'none'});
                ...
                {navEntreprise && navEntreprise !== 'groupoy' && navEntreprise !== 'yilmaz' && (() => {
                  ...
                  const KPI_FIL = {
                    ezel: [
                      ...
                      {l:'Chantiers Phase 3',v:'14',i:'🏗️',c:'#10b981',sub:'6.82M€ · exécution'},
                    ...
                    echafaudage: [
                      {l:'Chantiers montage',v:'8',i:'🏗️',c:filAcc,sub:'actifs en ce moment'},
                    ...
                    etancheite: [
                      ...
                      {l:'Chantiers actifs',v:'6',i:'🏗️',c:'#10b981',sub:'exécution en cours'},
                    ...
                    yilmaz: [
                      ...
                      {l:'Missions actives',v:'12',i:'⚙️',c:'#8b5cf6',sub:'services partagés'},
                    ]
                  };
              ...
              {(() => { const alerts = getAlerts(); return alerts.length > 0 ? (
                <div style={{marginBottom:16,padding:'14px 18px',borderRadius:crmRd,background:'linear-gradient(135deg, rgba(128,128,128,0.10), rgba(128,128,128,0.04))',border:'1px solid #fecaca'}}>
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:6}}>
                    {alerts.slice(0,8).map((a,i) => (
                      <div key={i} onClick={()=>setOngletActif(a.module)} style={{display:'flex',alignItems:'center',gap:8,padding:'6px 10px',borderRadius:crmRd,background:$bgCard,border:'1px solid '+(a.type==='critical'?'rgba(239,68,68,0.22)':a.type==='warning'?'rgba(212,160,48,0.18)':'rgba(99,102,241,0.14)'),cursor:'pointer',fontSize:'0.78rem'}}
                ...
                <div style={{display:'flex', gap:8, marginBottom:24, flexWrap:'wrap'}}>
                  ...
                  <button onClick={() => { setDashboardVue('chantiers'); setDashboardCollabId(null); setDashboardChantierId(null); }} style={tabS(isCh)}>🏗️ Chantiers ({filChantiers.length})</button>
                </div>
                ...
                {dashboardVue === 'kpi' && (<>
                      ...
                      <div style={{padding:'12px 20px'}}>
                        {(() => {
                          ...
                          return <>
                            ...
                            <div style={{display:'flex', justifyContent:'space-between', padding:'12px 14px', background:'linear-gradient(135deg, rgba(128,128,128,0.10), rgba(128,128,128,0.04))', borderRadius:crmRd, marginTop:6}}>
                        ...
                        {ratios.map((r,i) => {
                          ...
                          <div key={i} style={{marginBottom:14}}>
                            ...
                            <div style={{position:'relative', height:8, background:$bgSub, borderRadius:crmRd, overflow:'hidden'}}>
                              <div style={{position:'absolute', left:`${(r.min - r.min*0.5)/(r.max*1.5-r.min*0.5)*100}%`, width:`${(r.max-r.min)/(r.max*1.5-r.min*0.5)*100}%`, height:'100%', background:'rgba(232,245,233,0.14)', borderRadius:crmRd}} />
                  ...
                  <div style={{display:'flex', gap:16, marginBottom:20, flexWrap:'wrap'}}>
                    ...
                    <div style={{...fCS, flex:'1 1 48%', boxShadow:'0 2px 16px rgba(0,0,0,0.03)', minWidth:340}}>
                      <div style={{...fHS, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                        <div style={{display:'flex', alignItems:'center', gap:8}}><div style={fTS}>🏗️ Chantiers actifs</div>{fYB}</div>
                  ...
                  {(() => {
                    ...
                    return (
                    <div style={{...fCS, marginBottom:20}}>
                      <div style={fHS}><div style={fTS}>⚠️ Alertes</div></div>
                      <div style={{padding:12}}>
                        {al.map((a,i) => (
                  ...
                  return (<div>
                    <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:24}}>
                      ...
                      <div style={{display:'flex', gap:8, alignItems:'center'}}>
                        ...
                        <button onClick={() => { setOngletActif('collaborateurs'); setCollabOngletId(emp.id);setCollabDetailTab('profil'); }} style={{padding:'6px 14px', borderRadius:crmRd, border:`1px solid ${$border}`, background:$bgCard, cursor:'pointer', fontSize:'0.85rem', fontWeight:600, color:'#2563eb'}}>✏️ Modifier dans YILMAZ RH →</button>
                      </div>
                    </div>
                    <div style={fCS}>
                      <div style={{...fHS, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                      ...
                      <div style={{padding:20}}>
                        ...
                        <div style={{fontSize:'0.92rem', fontWeight:700, color:$text, marginBottom:10}}>🏗️ Chantiers ({mesChantiers.length})</div>
                ...
                <div style={fCS}>
                  <div style={{...fHS, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                    <div style={{display:'flex', alignItems:'center', gap:8}}><div style={fTS}>🏗️ Chantiers — {fil.nom}</div>{fYB}</div>
                    <div style={{display:'flex', alignItems:'center', gap:8}}>
                ...
                {dashboardVue === 'chantier' && dashboardChantierId && (() => {
                  ...
                  const phases = [
                    ...
                    {label:'Exécution', pct: Math.min(100, Math.max(0, ch.avancement - 20) * 1.5), icon:'🏗️'},
                  ...
                  return (<div>
                    <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:24}}>
                      ...
                      <div style={{display:'flex', gap:8}}>
                        <button onClick={() => { setChantierForm({...ch}); setModalChantier('edit'); }} style={{padding:'6px 14px', borderRadius:crmRd, border:`1px solid ${$border}`, background:$bgCard, cursor:'pointer', fontSize:'0.85rem', fontWeight:600, color:'#2563eb'}}>✏️ Modifier</button>
                    ...
                    <div style={{...fCS, marginBottom:20}}>
                      <div style={{padding:'20px 24px', display:'flex', justifyContent:'space-between', alignItems:'flex-start', borderBottom:`1px solid ${$border}`}}>
                        <div>
                          <div style={{fontSize:'1.2rem', fontWeight:800, color:$text}}>🏗️ {ch.nom}</div>
                        ...
                        <div style={{display:'flex', alignItems:'center', gap:8}}>
                          {retard && <span style={{fontSize:'0.78rem', fontWeight:700, padding:'3px 10px', borderRadius:crmRd, background:$danger+'12', color:'#dc2626', border:'1px solid #fecaca'}}>⚠️ Retard</span>}
                          <span style={statutBadge(ch.statut)}>{ch.statut}</span>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div style={{position:'relative', marginTop:12}}>
                          ...
                          <div style={{width:'100%', height:6, background:$bgSub, borderRadius:3, overflow:'hidden'}}>
                            <div style={{height:'100%', borderRadius:3, background: retard ? 'rgba(239,68,68,0.30)' : 'rgba(34,197,94,0.22)', width:`${timePct}%`}} />
                          </div>
                        </div>)}
                      </div>
                    </div>
                    ...
                    <div style={{...fCS, marginBottom:20}}>
                      ...
                      <div style={{display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:12, padding:16}}>
                        {[
                          ...
                          const colors = {danger:{bg:'rgba(239,68,68,0.10)', border:'rgba(239,68,68,0.22)', text:'#dc2626', icon:'▲'}, warning:{bg:'rgba(212,160,48,0.12)', border:'rgba(212,160,48,0.26)', text:'#92400e', icon:'●'}, ok:{bg:'rgba(34,197,94,0.10)', border:'rgba(34,197,94,0.22)', text:'#166534', icon:'✓'}};
