            ...
            return (<>
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
                ...
                <div style={{display:'flex',gap:6}}>
                  {!posteEditMode ? (canEdit('postes') ? <><button onClick={() => { setPosteEditData({...p}); setPosteEditMode(true); }} style={{padding:'7px 14px',borderRadius:crmRd,border:`2px solid ${$accent}`,background:'transparent',color:$accent,fontWeight:600,fontSize:'0.82rem',cursor:'pointer',fontFamily:'inherit'}}>✎ Modifier</button><button onClick={() => setConfirmDelete({type:'poste',id:p.id,nom:p.titre||'Sans titre'})} style={{padding:'7px 14px',borderRadius:crmRd,border:`1px solid ${$danger}30`,background:$danger+'08',color:$danger,fontWeight:600,fontSize:'0.82rem',cursor:'pointer',fontFamily:'inherit'}}>⊘ Supprimer</button></> : null) : (<>
              ...
              <div style={{background:$bgCard,borderRadius:crmRd,padding:'20px 24px',border:`1px solid ${$border}`,marginBottom:16}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                  <div>
                    ...
                    <div style={{fontSize:'0.82rem',color:$textMut,marginTop:4}}>{fil?`${fil.icon} ${fil.nom}`:'◆ Yilmaz'} — {p.service||'—'}</div>
              ...
              {posteEditMode&&<div style={{background:$bgCard,borderRadius:crmRd,padding:'20px 24px',border:`1px solid ${$accent}30`,marginBottom:16}}>
                <div style={{fontSize:'0.72rem',fontWeight:600,color:$accent,textTransform:'uppercase',letterSpacing:'0.04em',marginBottom:14}}>✎ Modifier le poste</div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10,marginBottom:14}}>
                  ...
                  <div><label style={labelSt}>Filiale</label><select style={inputSt} value={ed.filialeId||''} onChange={e=>setPosteEditData({...ed,filialeId:e.target.value==='yilmaz'?'yilmaz':e.target.value?Number(e.target.value):'yilmaz'})}><option value="yilmaz">◆ Yilmaz</option>{filialesDynamiques.map(f=><option key={f.id} value={f.id}>{f.nom}</option>)}</select></div>
