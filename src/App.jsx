  ...
  render() {
    if (this.state.hasError) {
      ...
      return <div style={{padding:'20px 24px', margin:'16px 0', background:'rgba(239,68,68,0.08)', borderRadius:12, border:'1px solid rgba(239,68,68,0.35)', color:'#ef4444', fontSize:'0.92rem', width:'100%', boxSizing:'border-box'}}>
        ...
        <button onClick={() => this.setState({hasError: false, error: null})} style={{padding:'6px 14px', borderRadius:8, border:'1px solid rgba(239,68,68,0.4)', background:'transparent', color:'#dc2626', cursor:'pointer', fontSize:'0.85rem', fontWeight:600}}>Réessayer</button>
      </div>;
    }
...
const SimulateurRuches = () => {
  ...
  const loginAsEmail = (email) => {
    ...
    if (user) {
      setCurrentUser(user); setIsLoggedIn(true); setShowLoginModal(false); setOngletActif('aujourdhui');
  ...
  const canView = (ongletId) => {
    ...
    if (['crm_commercial', 'ezel_tableau', 'suivi_dossiers', 'kpi_dashboard', 'veille_ao', 'svc_kpi', 'planning_gantt', 'calendrier_svc', 'processus_svc', 'guide', 'presentation_groupe',
      ...
      'outils', 'tickets',
      'contrats', 'litiges', 'assurances', 'conformite', 'centre_echeances', 'documents', 'aujourdhui',
  ...
  const SERVICES_CONFIG = {
    'groupoy': {
      ...
      services: [
        { id: 'direction', label: 'Direction Générale', icon: '🏛️', modules: ['aujourdhui', 'dashboard', 'centre_echeances', 'documents', 'presentation_groupe', 'presentation', 'admin'] },
    ...
    'yilmaz': {
      ...
      services: [
        { id: 'direction', label: 'Direction Générale', icon: '🏛️', modules: ['aujourdhui', 'dashboard', 'centre_echeances', 'documents', 'presentation_groupe', 'presentation', 'roadmap'], desc: 'Pilotage stratégique, vision groupe, feuille de route' },
  ...
  const STATUTS_FACTEXT = {
    reception: { label:'📥 Réceptionnée', bg:'rgba(128,128,128,0.12)', color:'#374151', step:1 },
    validation_manager: { label:'👤 Validation manager', bg:'rgba(212,160,48,0.18)', color:'#92400e', step:2 },
    validation_daf: { label:'💼 Validation DAF', bg:'rgba(59,130,246,0.14)', color:'#1e40af', step:3 },
    bon_a_payer: { label:'✅ Bon à payer', bg:'rgba(34,197,94,0.14)', color:'#065f46', step:4 },
    payee: { label:'💰 Payée', bg:'rgba(34,197,94,0.14)', color:'#166534', step:5 },
    rejetee: { label:'❌ Rejetée', bg:'rgba(239,68,68,0.22)', color:'#991b1b', step:0 },
    litige: { label:'⚠️ Litige', bg:'rgba(212,160,48,0.18)', color:'#dc2626', step:0 }
  };

  const CONTRAT_STATUTS = {
    a_preparer: { label:'À préparer', icon:'📝', bg:'rgba(212,160,48,0.18)', color:'#92400e' },
    en_redaction: { label:'En rédaction', icon:'✍️', bg:'rgba(99,102,241,0.14)', color:'#4338ca' },
    envoye: { label:'Envoyé', icon:'📤', bg:'rgba(59,130,246,0.14)', color:'#1e40af' },
    signe: { label:'Signé', icon:'✅', bg:'rgba(34,197,94,0.14)', color:'#166534' },
    expire: { label:'Expiré', icon:'⏰', bg:'rgba(239,68,68,0.22)', color:'#991b1b' },
    resilie: { label:'Résilié', icon:'🚫', bg:'rgba(128,128,128,0.12)', color:'#6b7280' }
  };
  ...
  const MISSION_STATUTS = {
    en_attente: { label:'En attente', icon:'⏳', bg:'rgba(128,128,128,0.12)', color:'#6b7280' },
    en_cours: { label:'En cours', icon:'🔄', bg:'rgba(59,130,246,0.14)', color:'#1e40af' },
    livre: { label:'Livré', icon:'📦', bg:'rgba(212,160,48,0.18)', color:'#92400e' },
    valide: { label:'Validé', icon:'✅', bg:'rgba(34,197,94,0.14)', color:'#166534' },
    annule: { label:'Annulé', icon:'❌', bg:'rgba(239,68,68,0.22)', color:'#991b1b' }
  };
  const TACHE_STATUTS = {
    a_faire: { label:'À faire', icon:'📋', bg:'rgba(128,128,128,0.12)', color:'#6b7280' },
    en_cours: { label:'En cours', icon:'🔄', bg:'rgba(59,130,246,0.14)', color:'#1e40af' },
    en_revue: { label:'En revue', icon:'👀', bg:'rgba(212,160,48,0.18)', color:'#92400e' },
    terminee: { label:'Terminée', icon:'✅', bg:'rgba(34,197,94,0.14)', color:'#166534' }
  };
  ...
  const SourceLogo = ({name, size=16}) => {
    if (name === 'SPIGAO') return (<div style={{width:size, height:size, borderRadius:3, background:'linear-gradient(135deg, rgba(128,128,128,0.10), rgba(128,128,128,0.04))', display:'inline-flex', alignItems:'center', justifyContent:'center', flexShrink:0}}><span style={{fontSize:size*0.5, fontWeight:800, color:'#fff', lineHeight:1}}>SP</span></div>);
  ...
  const AO_STATUTS = {
    'Visite RDV pris': { color: '#d97706', bg: 'rgba(212,160,48,0.12)', icon: '📅' },
    'En préparation': { color: '#007ab5', bg: 'rgba(59,130,246,0.10)', icon: '📝' },
    'En attente de réponse': { color: '#2563eb', bg: 'rgba(59,130,246,0.14)', icon: '⏳' },
    'À présenter': { color: '#0284c7', bg: 'rgba(224,242,254,0.14)', icon: '📤' },
    'Remporté': { color: '#059669', bg: 'rgba(34,197,94,0.10)', icon: '🏆' },
    'Non retenu': { color: '#6b7280', bg: 'rgba(128,128,128,0.12)', icon: '❌' },
    'Courrier demande de précisions': { color: '#7c3aed', bg: 'rgba(139,92,246,0.10)', icon: '✉️' },
    'Suite Refus': { color: '#94a3b8', bg: 'rgba(128,128,128,0.12)', icon: '📨' },
  };

  // Veille AO
  const [veilleStatutsOrdre, setVeilleStatutsOrdre] = useState([
    { id: 'nouveau', label: 'Nouveau', icon: '🆕', color: '#64748b', bg: 'rgba(128,128,128,0.12)', groupe: 'nouveaux' },
    { id: 'a_regarder', label: 'À Regarder', icon: '🐟', color: '#d97706', bg: 'rgba(212,160,48,0.12)', groupe: 'suivis' },
    { id: 'en_attente_dce', label: 'En attente de DCE', icon: '📂', color: '#7c3aed', bg: 'rgba(139,92,246,0.10)', groupe: 'suivis' },
    { id: 'en_analyse', label: 'En Analyse', icon: '🔍', color: '#0369a1', bg: 'rgba(240,249,255,0.14)', groupe: 'suivis' },
    { id: 'go_a_traiter', label: 'GO – À Traiter', icon: '⭐', color: '#15803d', bg: 'rgba(34,197,94,0.10)', groupe: 'suivis' },
    { id: 'no_go', label: 'No Go', icon: '👎', color: '#be123c', bg: 'rgba(255,241,242,0.14)', groupe: 'archives' },
    { id: 'hors_cible', label: 'Hors cible', icon: '🚫', color: '#9ca3af', bg: 'rgba(128,128,128,0.12)', groupe: 'archives' },
  ]);
  ...
  const AO_PRIORITES = {
    'Haute': { color: '#e74c3c', bg: 'rgba(239,68,68,0.14)' },
    'Moyenne': { color: '#f39c12', bg: 'rgba(212,160,48,0.18)' },
    'Basse': { color: '#3498db', bg: 'rgba(59,130,246,0.14)' },
  };
  const AO_TYPES = {
    'Public': { color: '#27ae60', bg: 'rgba(34,197,94,0.14)' },
    'Privé': { color: '#8b5cf6', bg: 'rgba(139,92,246,0.14)' },
  };
  ...
  const FilialeFilterBar = () => {
    ...
    return (
      <div style={{display:'flex',alignItems:'center',gap:8,padding:'6px 20px',background:$bgSub,borderBottom:`1px solid ${$borderAlt}`,flexWrap:'wrap'}}>
        ...
        <button onClick={()=>setFilialeFilter([])} style={{padding:'4px 12px',borderRadius:crmRd,border:'1px solid '+(filialeFilter.length===0?'#8B6F47':'#d4cfc7'),background:filialeFilter.length===0?'#8B6F47':$bgCard,color:filialeFilter.length===0?'white':'#6b5d4d',fontSize:'0.72rem',fontWeight:600,cursor:'pointer',transition:'all 0.2s'}}>
          🏛️ Toutes
        </button>
        {FILIALE_FILTER_OPTIONS.map(f => {
          const active = filialeFilter.includes(f.id);
          return (
            <button key={f.id} onClick={()=>toggleFilialeFilter(f.id)} style={{padding:'4px 12px',borderRadius:crmRd,border:'1px solid '+(active?f.couleur:'#d4cfc7'),background:active?f.couleur+'18':$bgCard,color:active?f.couleur:'#6b5d4d',fontSize:'0.72rem',fontWeight:600,cursor:'pointer',transition:'all 0.2s'}}>
              {f.icon} {f.nom} {active && '✓'}
            </button>
          );
        })}
    ...
    <div style={isLoggedIn ? {minHeight:'100vh', background:crmTh.isDark?crmTh.bg:crmBg, padding:crmLayout==='sidebar'?'24px 28px 1rem 28px':'1rem', fontFamily:crmTh.font, transition:'background 0.3s', marginLeft:crmLayout==='sidebar'?(crmSideOpen?210:48):0} : {paddingTop:56, fontFamily:'Inter, system-ui, sans-serif', minHeight:'100vh'}}>
      ...
      <div style={{maxWidth:'1600px', margin:'0 auto'}}>
        ...
        <div style={{position:'sticky', top:0, zIndex:50, transition:'all 0.3s ease', opacity: headerCompact ? 1 : 0, pointerEvents: headerCompact ? 'auto' : 'none', maxHeight: headerCompact ? 60 : 0, overflow:'hidden', marginBottom: headerCompact ? 6 : 0, display:crmLayout==='sidebar'?'none':undefined, height:crmLayout==='sidebar'?0:undefined}}>
          <div style={{background:$bgSub, opacity:0.97, backdropFilter:'blur(12px)', WebkitBackdropFilter:'blur(12px)', padding:'4px 4px', borderRadius:0}}>
              ...
              <div style={{display:'flex', gap:4, flexShrink:0}}>
                {[{key:'widgets', label:'📊'}, {key:'services', label:'🏢'}, {key:'groupe', label:'🐝'}].map(tab => (
                  <div key={tab.key} onClick={() => setYilmazVue(tab.key)}
                    style={{padding:'3px 8px', borderRadius:crmRd, fontSize:'0.8rem', fontWeight:700, background: yilmazVue === tab.key ? '#8B6F47' : $border, color: yilmazVue === tab.key ? 'white' : '#b0a08a', cursor:'pointer', transition:'all 0.2s'}}>
                    {tab.label}
                  </div>
                ))}
              </div>
                      ...
                      <div style={{display:'flex',gap:2}}>
                        {(row.hasXS ? [{id:'lg',l:'L'},{id:'md',l:'M'},{id:'sm',l:'S'},{id:'xs',l:'XS'}] : [{id:'lg',l:'L'},{id:'md',l:'M'},{id:'sm',l:'S'}]).map(opt=>(
                          <button key={opt.id}
                            ...
                            style={{width:26,height:22,borderRadius:Math.max(crmRd-2,0),border:'1px solid '+(row.state===opt.id?crmAcc:$border),background:row.state===opt.id?crmAcc+'18':'transparent',color:row.state===opt.id?crmAcc:'#b0a08a',fontSize:'0.65rem',fontWeight:row.state===opt.id?700:400,cursor:'pointer',fontFamily:'inherit',transition:'all 0.12s'}}
                          >{opt.l}</button>
                        ))}
                      </div>
                    </div>
          ...
          {showChangePassword && (
            <div style={{position:'absolute', right:40, top:80, background:$bgCard, borderRadius:crmRd, padding:20, boxShadow:'0 10px 40px rgba(0,0,0,0.2)', zIndex:100, width:280}} onClick={e => e.stopPropagation()}>
              ...
              <div style={{display:'flex', gap:6}}>
                ...
                <button onClick={() => { setShowChangePassword(false); setPasswordMsg(''); }} style={{padding:'6px 12px', background:$border, border:'none', borderRadius:crmRd, fontSize:'0.9rem', cursor:'pointer'}}>✕</button>
              </div>
            </div>
          )}
        </div>{/* end header bar */}
        ...
        {crmPal && crmLayout==='cartes' && headerCompact && <div onClick={()=>setCrmPal(false)} style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'transparent',zIndex:99999}}>
          <div onClick={e=>e.stopPropagation()} style={{position:'fixed',top:52,right:20,width:320,background:$bgCard,border:`1px solid ${$borderAlt}`,borderRadius:crmRd,padding:20,boxShadow:'0 12px 40px rgba(0,0,0,0.15)',zIndex:99999,maxHeight:'80vh',overflow:'auto'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:16}}>
              ...
              {!crmTh.isDark&&<div><div style={{fontSize:'0.72rem',fontWeight:700,color:$text,marginBottom:8}}>Couleur de fond {crmTint&&<span style={{fontSize:'0.65rem',fontWeight:400,color:crmAcc}}>{String.fromCodePoint(0x2014)} {CRM_TINTS[crmTint]?.n}</span>}</div><div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:5}}>{Object.entries(CRM_TINTS).map(([id,tn])=>(<button key={id} onClick={()=>setCrmTint(crmTint===id?null:id)} style={{padding:'6px 4px',borderRadius:crmRd,border:'2px solid '+(crmTint===id?crmAcc:$border),background:crmTint===id?crmAcc+'12':'transparent',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:3,boxShadow:crmTint===id?'0 0 0 1px '+crmAcc:'none',transition:'all 0.15s'}}><div style={{width:18,height:18,borderRadius:'50%',background:tn.c,border:'1px solid #e8e4de'}}/><span style={{fontSize:'0.6rem',color:crmTint===id?$text:'#b0a08a',fontWeight:crmTint===id?600:400}}>{tn.n}</span></button>))}</div><div style={{marginTop:8,opacity:crmTint?1:0.35,pointerEvents:crmTint?'auto':'none'}}><div style={{fontSize:'0.65rem',color:'#b0a08a',marginBottom:4}}>Intensité du fond</div><div style={{display:'flex',gap:2,background:$bgSub,borderRadius:crmRd,padding:3,border:'1px solid #e8e4de',width:'fit-content'}}>{[{i:0,l:'Léger'},{i:1,l:'Moyen'},{i:2,l:'Fort'}].map(lv=><button key={lv.i} onClick={()=>setCrmTintLvl(lv.i)} style={{padding:'4px 12px',borderRadius:Math.max((crmRd)-2,0),border:'none',cursor:'pointer',background:crmTintLvl===lv.i?crmAcc:'transparent',color:crmTintLvl===lv.i?'#fff':'#b0a08a',fontWeight:crmTintLvl===lv.i?600:400,fontSize:'0.72rem',fontFamily:'inherit',transition:'all 0.15s'}}>{lv.l}</button>)}</div></div></div>}
              <div><div style={{fontSize:'0.72rem',fontWeight:700,color:$text,marginBottom:8}}>Arrondi des bords</div><div style={{display:'flex',gap:4}}>{CRM_RAD.map(opt=>(<button key={opt.id} onClick={()=>setCrmRadius(opt.id)} style={{flex:1,padding:'10px',borderRadius:crmRd,border:'1px solid '+(crmRadius===opt.id?crmAcc:$border),background:crmRadius===opt.id?crmAcc+'12':'transparent',cursor:'pointer',fontFamily:'inherit',display:'flex',flexDirection:'column',alignItems:'center',gap:4,transition:'all 0.15s'}}><div style={{width:28,height:20,borderRadius:opt.v+'px',border:'2px solid '+(crmRadius===opt.id?crmAcc:'#b0a08a'),transition:'all 0.15s'}}/><span style={{fontSize:'0.72rem',fontWeight:crmRadius===opt.id?600:400,color:crmRadius===opt.id?'#2d2216':'#b0a08a'}}>{opt.l}</span><span style={{fontSize:'0.58rem',color:'#b0a08a'}}>{opt.d}</span><span style={{fontSize:'0.55rem',color:crmRadius===opt.id?crmAcc:'#c4bfb6',fontWeight:500}}>{opt.s}</span></button>))}</div></div>
              <div><div style={{fontSize:'0.72rem',fontWeight:700,color:$text,marginBottom:8}}>Navigation</div><div style={{display:'flex',gap:4}}>{[{id:'cartes',l:'Cartes',sub:'Par cartes',icon:'☰'},{id:'sidebar',l:'Sidebar',sub:'Panneau latéral',icon:'◫'}].map(opt=>(<button key={opt.id} onClick={()=>{setCrmLayout(opt.id);if(opt.id==='sidebar'&&!navEntreprise){setNavEntreprise('groupoy');setOngletActif('dashboard');}}} style={{flex:1,padding:'10px',borderRadius:crmRd,border:'1px solid '+(crmLayout===opt.id?crmAcc:$border),background:crmLayout===opt.id?crmAcc+'12':'transparent',cursor:'pointer',fontFamily:'inherit',display:'flex',flexDirection:'column',alignItems:'center',gap:4,transition:'all 0.15s'}}><span style={{fontSize:'1rem'}}>{opt.icon}</span><span style={{fontSize:'0.72rem',fontWeight:crmLayout===opt.id?600:400,color:crmLayout===opt.id?$text:'#b0a08a'}}>{opt.l}</span><span style={{fontSize:'0.58rem',color:$textMut}}>{opt.sub}</span></button>))}</div></div><div><div style={{fontSize:'0.72rem',fontWeight:700,color:$text,marginBottom:8}}>Affichage navigation</div><div style={{display:'flex',flexDirection:'column',gap:6}}>{[{label:'Filiales nav',keep:navKeepFil,set:setNavKeepFil},{label:'Services',keep:navKeepCards,set:setNavKeepCards}].map(row=>(<div key={row.label} style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:6}}><span style={{fontSize:'0.68rem',color:'#b0a08a',flex:1}}>{row.label}</span><div style={{display:'flex',gap:2}}>{[{id:false,l:'Auto'},{id:true,l:'Fixe'}].map(opt=>(<button key={String(opt.id)} onClick={()=>row.set(opt.id)} style={{padding:'3px 10px',borderRadius:Math.max(crmRd-2,0),border:'1px solid '+(row.keep===opt.id?crmAcc:$border),background:row.keep===opt.id?crmAcc+'18':'transparent',color:row.keep===opt.id?crmAcc:'#b0a08a',fontSize:'0.65rem',fontWeight:row.keep===opt.id?700:400,cursor:'pointer',fontFamily:'inherit',transition:'all 0.12s'}}>{opt.l}</button>))}</div></div>))}</div></div><div style={{display:'flex',flexDirection:'column',gap:6}}>
                  <div style={{fontSize:'0.72rem',fontWeight:700,color:$text,marginBottom:4}}>Taille des cartes</div>
                  {[
                    {label:'Filiales (nav)',state:filCardSize,setter:'fil'},
                    {label:'Services',     state:svcCardSize,setter:'svc'},
                ...
                return (
                  ...
                  >
                    <div style={{height: filCardSize==='sm'?72:filCardSize==='md'?96:120, background: gradients[key], position:'relative', display:'flex', alignItems:'center', justifyContent:'center'}}>
                      ...
                      {isActive && (
                        <div style={{position:'absolute', top:12, right:12, background:$bgCard, borderRadius:'50%', width:20, height:20, display:'flex', alignItems:'center', justifyContent:'center'}}>
                    ...
                    <div key={svc.id} style={{display:'grid', gridTemplateColumns:'1fr 70px 80px', gap:0, alignItems:'center', padding:'7px 16px', borderBottom:`1px solid ${$border}`}}>
                      ...
                      <div style={{display:'flex', justifyContent:'center'}}>
                        <div role="button" tabIndex={0} onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); const sid = svc.id; setHiddenServicesYilmaz(prev => prev.includes(sid) ? prev.filter(x => x !== sid) : [...prev, sid]); }}
                          style={{width:40, height:22, borderRadius:crmRd, background: hiddenServicesYilmaz.includes(svc.id) ? $border : '#059669', cursor:'pointer', position:'relative', transition:'background 0.2s', userSelect:'none'}}>
                    ...
                    return (
                      <div key={modId}
                        ...
                        style={{
                          ...
                          background: isActiveModule ? `${mod.color}08` : $bgCard,
                          transition:'all 0.25s ease',
      ...
      {ongletActif === 'aujourdhui' && <TabAujourdhui {...{ $accent, $bgCard, $bgSub, $border, $shadow, $shadowLg, $text, $textMut, $textSec, $danger, $warn, $success, $info, crmRd, currentUser, filNom, filterByFiliale, setOngletActif, assData, ctrData, autoData, veilleAO, factExtData, chantiers, employes, filialesEnrichies }} />}
      ...
      {ongletActif === 'parc_info' && <TabParcInfo {...{ $bgSub, $bgCard, $border, $shadow, $shadowLg, $text, $textMut, $textSec, FILIALE_FILTER_OPTIONS, crmRd, empNom, employes, parcInfoData, parcInfoDetail, parcInfoEdit, parcInfoFilter, setParcInfoData, setParcInfoDetail, setParcInfoEdit, setParcInfoFilter }} />}

      {/* ══════ MODULE: DONNÉES DE RÉFÉRENCE ══════ */}
      {ongletActif === 'donnees_ref' && <TabDonneesRef {...{ $bgSub, $accent, $bgCard, $border, $danger, $shadow, $shadowLg, $text, $textMut, $textSec, autoData, ca, chantiers, collaborateurs, crmRd, drTab, employes, filiales, filialesDynamiques, niveau, setCollabDetailTab, setCollabOngletId, setDrTab, setEmployeForm, setModalEmploye, setOngletActif }} />}
